...
if __name__ == "__main__":
    ...
    if not os.path.exists(dataset_path):
        # Fallback to mock dataset if not seeded yet to prevent crash
        print(f"dataset.csv not found at {dataset_path}. Generating mock dataset...", file=sys.stderr)
    ...
    joblib.dump(dt_syn, os.path.join(models_dir, 'dt_model_syn.joblib'))
    
    rf_syn = train_rf(X_train_syn, y_train)
    joblib.dump(rf_syn, os.path.join(models_dir, 'rf_model_syn.joblib'))
    
    xgb_syn = train_xgb(X_train_syn, y_train)
    joblib.dump(xgb_syn, os.path.join(models_dir, 'xgb_model_syn.joblib'))
    
    print(f"All models trained and exported to {models_dir} directory.")
